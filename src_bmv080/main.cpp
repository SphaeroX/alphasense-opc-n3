...
void setup()
{
  ...
  // Initialize I2C bus for the BMV080
  Wire.begin();
  if (!bmv080.begin(BMV080_ADDR, Wire)) {
    Serial.println("BMV080 not detected. Program halted.");
    while (1)
      ;
  }
...
void loop()
{
  ...
  else
  {
    ...
    if (!opcOk) Serial.println("OPC-N3 read failed");
    if (!bmvOk) Serial.println("BMV080 read failed");
